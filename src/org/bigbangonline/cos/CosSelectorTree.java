...
/**
 * The Class CosSelectorTree.
 */
public class CosSelectorTree extends JTree{
	...
	/**
	 * Instantiates a new cos selector tree.
	 */
	public CosSelectorTree(){
	 ...
	 */
	public CosDataStructure getSelectedObject(){
		try{
			if(((DefaultMutableTreeNode)getSelectionPath().getLastPathComponent()).getUserObject() instanceof CosDataStructure){
				return (CosDataStructure)((DefaultMutableTreeNode)getSelectionPath().getLastPathComponent()).getUserObject();
			}
			return null;
	 ...
	 */
	public void setCurrentState(Vector<CosDataStructure> vector){
		...
		while(itr.hasNext()){
			...
			CosTreeNode node = new CosTreeNode(cds);
			if(cds.getPath().equals("/PUBLIC/")){
				publicNode.add(node);
			}else if(cds.getPath().equals("/SHARED/")){
				sharedNode.add(node);
			}else if(cds.getPath().equals("/USER/")){
				userNode.add(node);
			}
		}
