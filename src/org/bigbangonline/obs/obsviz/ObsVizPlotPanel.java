...
/**
 * The Class ObsVizPlotPanel.
 */
public class ObsVizPlotPanel extends JPanel implements MouseMotionListener, MouseListener{
    ...
    /**
     * Instantiates a new obs viz plot panel.
     *
     * @param frame the frame
     * @param ds the ds
     */
    public ObsVizPlotPanel(ObsVizPlotFrame frame
	...
	public void setCurrentState(CustomPlotData customPlotData){
		...
		Vector<ObsDataStructure> obsVector = ds.getObsDataStructureVectorSelected();

		Iterator<ObsDataStructure> itrObs = obsVector.iterator();
		while(itrObs.hasNext()){
			ObsDataStructure ods = itrObs.next();
			imax += ods.getQuantityDataStructureVector().size();
			kmax = 1;
		}
		...
		xLabels = new String[ds.getObsDataStructureVectorSelected().size()];
		
		int counter = 0;
		int runCounter = 0;
		itrObs = obsVector.iterator();
		while(itrObs.hasNext()){
			ObsDataStructure ods = itrObs.next();
			xLabels[runCounter] = ods.getName();
			...
			while(quantityItr.hasNext()){
				...
				x[counter][0] = runCounter+1;
				y[counter][0] = oqds.getMid();
				yerrmin[counter][0] = oqds.getMin();
				yerrmax[counter][0] = oqds.getMax();
	    ...
	    minorX = false;
	    xmin = 0;
		xmax = ds.getObsDataStructureVectorSelected().size()+1;
		xtickIntervals = (int)xmax;
	...
	public void paintComponent(Graphics g){
    	...
    	plotter.plot(plotmode,x1,y1,x2,y2,
                  ...
                  npoints,doscalex,doscaley,doplot,doclip,true,xLabels,xmin,xmax,ymin,ymax,
    	...
    	if(showWindow && mouseDragging){
			...
			plotter.plot(plotmode,newX1,newY1,newX2,newY2,
		                  kmax,imax,mode,
		                  dotSize,xlegoff,ylegoff,xdplace,ydplace,
		                  npoints,doscalex,doscaley,doplot,doclipMagnify,true,xLabels,xmin,xmax,ymin,ymax,
		                  delxmin,delxmax,delymin,delymax,
		                  lcolor,bgcolor,axiscolor,legendfg,framefg,
		                  dropShadow,legendbg,labelcolor,ticLabelColor,
		                  xtitle,ytitle,curveTitle,logStyle,ytickIntervals,
